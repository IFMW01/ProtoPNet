...
original_img = np.save(os.path.join(save_analysis_path, 'original_img.npy'),
                                     images_test.detach().cpu().numpy())
original_img = images_test.detach().cpu().numpy()

##### MOST ACTIVATED (NEAREST) 10 PROTOTYPES OF THIS IMAGE
makedir(os.path.join(save_analysis_path, 'most_activated_prototypes'))

log('Most activated 3 prototypes of this image:')
...
for i in range(1,4):
    ...
    upsampled_activation_pattern = cv2.resize(activation_pattern, dsize=tuple(original_img.shape[-2:]),
    ...
    #plt.axis('off')
    np.save(os.path.join(save_analysis_path, 'most_activated_prototypes',
                            'most_highly_activated_patch_by_top-%d_prototype.npy' % i),
               high_act_patch)
